...
with col_strategy:
    ...
    with tab1:
        st.markdown(
            ...
            '<p class="phase-body">Initiate substation upgrades at the 5 Congested nodes where '
            'i-DE (Iberdrola\'s own distribution subsidiary) manages the grid — '
            'IBE_020 (AP-9, A Coruña), IBE_022 (AP-7N, Castellón), IBE_024 (A-3, Madrid–Valencia), '
            'IBE_030 (N-332, Alicante), IBE_031 (AP-68, Bilbao). '
            'No third-party approval needed — cuts the standard 18–24 month process to weeks. '
            'Each node earns a grid upgrade fee + charging revenue simultaneously.</p>'
            '</div>',
            unsafe_allow_html=True,
        )
        st.markdown(
        ...
        st.markdown(
            ...
            'Moderate-grid stations across all corridors. Re-run composite scoring quarterly '
            'as new DGT charger data is published. Expand focus to high-CAGR Emerging provinces: '
